...
export const POST = async ({ request }) => {
    ...
    const componenteSelecionadoID = await request.json()
    const formData = new FormData();
    formData.append("wstoken", WSTOKEN_MOODLE);
    formData.append("wsfunction", "mod_survey_get_surveys_by_courses");
    formData.append("moodlewsrestformat", "json");
    formData.append("courseids[0]", componenteSelecionadoID);
